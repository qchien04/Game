...
public:
    explicit UDPNetWorkDualChannel(SOCKET sock) 
        : socket(sock),
    ...
    void start() {
        ...
        send_thread = std::thread(&UDPNetWorkDualChannel::send_worker, this);
        retry_thread = std::thread(&UDPNetWorkDualChannel::retry_worker, this);
        ack_thread = std::thread(&UDPNetWorkDualChannel::ack_worker, this);
    }
